...
#     results = face_recognition.compare_faces(embeddings, unknown_encoding, tolerance=0.4)
...
#                     dbembedding = np.array(document.get('encoding'))
#                     print("dbembedding",dbembedding)
        ...
        if image_file:
            ...
            def authenticate_finger_id(fingerid):
                ...
                if document:
                    ...
                    if result:
                        payload = {'uid': username, 'role': "voter"}
                        secretkey = "abc"
                        token = jwt.encode(payload, secretkey, algorithm='HS256')
                        return token
                    else:
